...
if ($_GET['categoryid']>'')
{
$event_url="https://api.memberfind.me/v1/evt?org=12918&all&sdp=".strtotime("Today")."&edp=".strtotime("Today + 1 week")."&Z=".time()."&grp=".$_GET['categoryid'];
}
else
{
$event_url="https://api.memberfind.me/v1/evt?org=12918&all&sdp=".strtotime("Today")."&edp=".strtotime("Today + 1 week")."&Z=".time();
...
$googleurl="'http://www.google.com/calendar/event?action=TEMPLATE&text=".$event->ttl."&dates=".date('Ymd',$event->sdp)."T".date('His',$event->sdp)."/".date('Ymd',$event->edp)."T".date('His',$event->edp)."&details=For details on this and other events, visit the Duvall Chamber of Commerce event calendar at http://duvallchamberofcommerce.com/duvall-events/&location=".$event->adn."'";
    ...
    <tr>
    <td>
    <p><b>Add to calendar:</b>
    <br/>
    <a href='http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['REQUEST_URI'])."/calitem.php?start=".$event->sdp."&amp;end=".$event->edp."&amp;desc=".$event->url."&amp;sub=".$event->ttl."&amp;loc=".$event->adn."' target=_new>Outlook | iCal</a>
...
// So we artificially create the pubdate by subtracting 1 week from the start date for the event
    ...
    echo "<item>";
	echo "<title>".strip_tags($event->ttl)."</title>";
